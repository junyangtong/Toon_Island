Shader "Island/Water"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Assets//Water/Shaders/Includes/Common.hlsl"
            ...
            {   
                ...
                //光照计算
                    //深度颜色渐变
                    SceneData scene;
                    SceneDepth depth = SampleDepth( i.scrPos);
                    float backgroundDepth = depth.eye;
                    float surfaceDepth = i.scrPos.w;
                    //Distance to opaque geometry in normal direction
                    float3 viewDelta = GetCurrentViewPosition() - i.posWS.xyz;
                    scene.positionWS = ReconstructWorldPosition(i.vertex, viewDelta, depth);
	                scene.verticalDepth = DepthDistance(i.posWS, scene.positionWS, nDirWS);
                    float viewWaterDepth = backgroundDepth - surfaceDepth;	                                //深度差值
                    float fog = saturate(scene.verticalDepth * _DepthRange);                        //取绝对值返回比例
                    float4 depthCol = lerp(_WaterShallowColr, _WaterDeepColr, fog);            //控制浅水区和深水区颜色
                    //折射
                    float4 refraction = SAMPLE_TEXTURE2D(_CameraOpaqueTexture, smp_Point_Repeat, warpScreenPos);
                    float alphaRange = saturate(scene.verticalDepth * _AlphaRange);
                    float3 diffColor =  lerp(refraction.rgb,depthCol.rgb,alphaRange) * light.color.rgb  * _Diffcol.rgb;
                    //岸边浪花
                    float4 foamTexCol = SAMPLE_TEXTURE2D(_FoamTex,smp_Point_Repeat,float2(sin( _Time.y * _Foam.y + min(_Foam.x, fog)/_Foam.x),0.5));
                    float foamRange = 1 - (min(_Foam.z , fog) / _Foam.z );                     // * step(fog,_Edge)// * step(fog,_Edge) * step(ratioZ,_Edge)
